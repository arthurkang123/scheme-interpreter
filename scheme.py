...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        ev_operands = operands.map(lambda operand: scheme_eval(operand, env))
        return scheme_apply(self, ev_operands, env)
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    v_first, v_next = expressions.first, expressions.second
    v_eval = scheme_eval(v_first, env, v_next is nil)
    if v_next is nil:
        return v_eval
    if v_first == 0 and type(v_first) == int:
        return do_and_form(v_next, env)
    if v_eval == False:
        return False
    return do_and_form(v_next, env)
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    bind, frml, val = bindings, nil, nil
    while not(bind is nil):
        bn = bind.first
        check_form(bind.first.second, 1, 1)
        frml = Pair(bn.first, frml)
        val = Pair(scheme_eval(bn.second.first, env), val)
        bind = bind.second
    check_formals(frml)
    return env.make_child_frame(frml, val)
    # END PROBLEM 15
